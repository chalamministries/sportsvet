...
public class FFmpegQueryExtension {
    ...
    fun cutVideo_2(inputVideoPath: String, startTime: String?, endTime: String?, output: String): Array<String> {
    ...
    fun addTextOnVideo(inputVideo: String, textInput: String, posX: Float?, posY: Float?, fontPath: String, isTextBackgroundDisplay: Boolean,
        fontSize: Int,
        fontcolor: String,
        output: String
    ): Array<String> {
...
//            add("anullsrc")
            add("-filter_complex")
//            add("[0:v][1:v]hstack=inputs=2[v]; [0:a][1:a]amerge[a]")
    ...
    private fun result(paths: ArrayList<Paths>){
        val inputs = "ffmpeg -i input-0.mp4 -i input-1.mp4 -i input-2.mp4 -i input-3.mp4 -i input-4.mp4 -f lavfi -t 0.1 -i anullsrc=channel_layout=mono:sample_rate=44100 -filter_complex \"[0:v]scale=1920:1080,setsar=1/1[v0];[1:v]scale=1920:1080,setsar=1[v1];[2:v]scale=1920:1080,setsar=1[v2];[3:v]scale=1920:1080,setsar=1[v3];[4:v]scale=1920:1080,setsar=1[v4];[v0][5][v1][1:a][v2][2:a][v3][3:a][v4][4:a]concat=n=5:v=1:a=1[v][a]\" -map \"[v]\" -map \"[a]\" -movflags +faststart output.mp4"
        //return inputs.trim().toArray(arrayOfNulls<String>(inputs.size))

        //-filter_complex "[0:v]scale=1920:1080,setsar=1/1[v0];[1:v]scale=1920:1080,setsar=1[v1];[2:v]scale=1920:1080,setsar=1[v2];[3:v]scale=1920:1080,setsar=1[v3];[4:v]scale=1920:1080,setsar=1[v4];[v0][5][v1][1:a][v2][2:a][v3][3:a][v4][4:a]concat=n=5:v=1:a=1[v][a]" -map "[v]" -map "[a]" -movflags +faststart output.mp4

...
class ShadowMerger(private val context: Context, private val callBack: ProcedureCallBack):
    FFmpegCallBack {
    ...
    fun combineVideosProcess(mediaList: ArrayList<Media>) {
        ...
        val query = combineVideos(mediaList, output.path)
...
//            "[0:v]scale=$str8,setsar=1[v0];[1:v]scale=$str8,setsar=1[v1];[v0][0:a][v1][1:a]concat=n=2:v=1:a=1",
    ...
    private fun combineVideos11(medias: ArrayList<Media>, output: String): Array<String> {
...
//            "anullsrc=channel_layout=stereo:sample_rate=48000",
            ...
            "-filter_complex",
            "[0:v]scale=$scale:force_original_aspect_ratio=decrease,pad=$scale:-1:-1,setsar=1,fps=30,format=yuv420p[v0];"+
                    "[1:v]scale=$scale:force_original_aspect_ratio=decrease,pad=$scale:-1:-1,setsar=1,fps=30,format=yuv420p[v1];"+
                    "[2:v]scale=$scale:force_original_aspect_ratio=decrease,pad=$scale:-1:-1,setsar=1,fps=30,format=yuv420p[v2];"+
        ...
        //"scale=iw*min($width/iw\\,$height/ih):ih*min($width/iw\\,$height/ih), pad=$scale1:($width-iw*min($width/iw\\,$height/ih))/2:($height-ih*min($width/iw\\,$height/ih))/2"
...
//        -map "[v]" -map "[a]" -c:v libx264 -c:a aac -movflags +faststart output.mp4
    ...
    private fun combineVideos(medias: ArrayList<Media>, output: String): Array<String> {
        width = medias[0].width
        height = medias[0].height
